...
function run_challenges() {
  ...
  var myEmail = $('#email').val('brocafleur@gmail.com');


  // Ex 3. Replace the email hint (next to the input) with 'This is my email now'
  //       The text should be emphasized using a <strong> tag
  var newEmailHint = $('#email-hint').html('<strong>This is my email now</strong>');

  // Ex 4. Add the .blue CSS class to the table header cells (th elements)
  var newColor = $('table th').addClass('blue');
  ...
  var newTeam = $('table tbody').append('<tr><td>15</td>, <td>new team</td>, <td>0</td></tr>');
  ...
  var newColorBg = $('th').css({ 'background-color':'#DDF4FF'});


  // Ex 9. Translate the #results list 20px downward (animation would be great ^_^)
  var newPositionResults = $('#results').animate({'top':'+=20px'});


  // Ex 10. Remove the "Email:" label from the DOM
  var removeEmailLabel = $('label').remove();
